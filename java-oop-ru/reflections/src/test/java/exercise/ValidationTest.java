...
class ValidationTest {
    ...
    void advancedValidateTest() {
        Address address3 = new Address("USA", null, null, null, "1");
        Map<String, List<String>> map = Validator.advancedValidate(address3);
        String expd = "{country=[length less than4], city=[can not be null],"
                + " street=[can not be null], houseNumber=[can not be null]}";
        assertEquals(expd, map.toString());
    }
    // END
}
